...
class HTTPSFixture(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path != "/hostsearch/" or query.get("q") != [TARGET]:
            body = b"fixture request rejected\n"
            self.send_response(404)
        else:
            body = RESULTS.encode("ascii")
...
def main() -> None:
    ...
    parser.add_argument("--fixture-ip", default="172.31.0.53")
    ...
    print(
        f"AEGIS_SUBFINDER_PASSIVE_FIXTURE_READY {API_HOST} {args.fixture_ip}",
        flush=True,
    )
