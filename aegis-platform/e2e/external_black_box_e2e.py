...
def csrf(session:requests.Session)->str:
 data=require(session.get(f'{DJANGO_URL}/auth/csrf/',timeout=15,verify=VERIFY_TLS),{200},'CSRF bootstrap')
...
def main()->int:
 ...
 require(session.post(f'{DJANGO_URL}/auth/login/',json={'email':email,'password':password},headers=headers,timeout=20),{200},'Login')
...
if __name__=='__main__':
 ...
 except Exception as exc:print(f'EXTERNAL_REAL_E2E=FAIL: {exc}',file=sys.stderr);raise
