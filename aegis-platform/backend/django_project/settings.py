...
MIDDLEWARE = [
    ...
    'django.middleware.csrf.CsrfViewMiddleware', 'django.contrib.auth.middleware.AuthenticationMiddleware', 'django_project.audit.middleware.EnterpriseAuditMiddleware',
...
WSGI_APPLICATION='django_project.wsgi.application'; ASGI_APPLICATION='django_project.asgi.application'; DATABASES={'default':env.db('DATABASE_URL')}; DATABASES['default']['CONN_MAX_AGE']=60
if DATABASES['default']['ENGINE'].endswith(('postgresql', 'mysql')): DATABASES['default']['OPTIONS']={'connect_timeout':10}
...
CORS_ALLOWED_ORIGINS=env('CORS_ALLOWED_ORIGINS'); CORS_ALLOW_CREDENTIALS=True; CORS_ALLOW_ALL_ORIGINS=False; CSRF_TRUSTED_ORIGINS=env('CSRF_TRUSTED_ORIGINS'); AUTH_COOKIE_SECURE=env('AUTH_COOKIE_SECURE') if 'AUTH_COOKIE_SECURE' in os.environ else (not DEBUG); AUTH_COOKIE_SAMESITE='Lax'; AUTH_COOKIE_HTTPONLY=True; AUTH_ACCESS_COOKIE='aegis_access'; AUTH_REFRESH_COOKIE='aegis_refresh'; SESSION_COOKIE_SECURE=AUTH_COOKIE_SECURE; SESSION_COOKIE_HTTPONLY=True; SESSION_COOKIE_SAMESITE='Lax'; CSRF_COOKIE_SECURE=AUTH_COOKIE_SECURE; CSRF_COOKIE_SAMESITE='Lax'; CSRF_COOKIE_HTTPONLY=False
...
REPORT_RETENTION_DAYS=env('REPORT_RETENTION_DAYS')
if not 1 <= REPORT_RETENTION_DAYS <= 3650: raise ImproperlyConfigured('REPORT_RETENTION_DAYS must be between 1 and 3650.')
CELERY_BROKER_URL=env('CELERY_BROKER_URL'); CELERY_RESULT_BACKEND=env('CELERY_RESULT_BACKEND'); CELERY_ACCEPT_CONTENT=['json']; CELERY_TASK_SERIALIZER='json'; CELERY_RESULT_SERIALIZER='json'; CELERY_TIMEZONE='UTC'; CELERY_TASK_TRACK_STARTED=True; CELERY_TASK_TIME_LIMIT=3600; CELERY_WORKER_PREFETCH_MULTIPLIER=1; CELERY_BEAT_SCHEDULER='django_celery_beat.schedulers:DatabaseScheduler'; CHANNEL_LAYERS={'default':{'BACKEND':'channels_redis.core.RedisChannelLayer','CONFIG':{'hosts':[REDIS_URL]}}}
