...
from .models import ContinuousAssuranceSchedule, Notification, ReportSchedule, CloudDiscoveryRun
...
def execute_report_schedule(schedule_id: str):
    ...
    schedule.next_run = timezone.now() + timedelta(minutes={'daily': 1440, 'weekly': 10080, 'monthly': 43200}.get(schedule.frequency, 60))
...
def send_notification(notification_id: str):
    ...
    try:
        if n.channel == Notification.Channel.EMAIL:
            if not n.user or not n.user.email:
                raise ValueError('Email notification requires a recipient user')
            ...
            if not url:
                raise ValueError('Webhook-style notification requires payload.url')
            response = requests.post(url, json=n.payload.get('body', n.payload), timeout=15)
            response.raise_for_status()
        else:
            raise ValueError(f'Unsupported notification channel: {n.channel}')
        n.status = Notification.Status.SENT; n.sent_at = timezone.now(); n.last_error = ''
        n.save(update_fields=['attempts', 'status', 'sent_at', 'last_error'])
        return {'status': 'sent', 'notification_id': notification_id}
    except Exception as exc:
        n.status = Notification.Status.FAILED; n.last_error = str(exc)
        n.save(update_fields=['attempts', 'status', 'last_error'])
        raise
...
def run_continuous_assurance(schedule_id: str):
    ...
    if not task:
        raise ValueError(f'Unsupported assurance engine: {schedule.engine}')
    asset = schedule.project.assets.filter(is_active=True).first()
    if not asset:
        raise ValueError('Continuous assurance requires an active project asset')
    scan = Scan.objects.create(project=schedule.project, name=f'Continuous assurance {schedule.engine}', scan_type=schedule.scan_type, asset=asset, engines=[schedule.engine], depth='quick', config=asset.configuration or {}, initiated_by_id=schedule.created_by_id)
    result = task.delay(str(scan.id))
    schedule.last_run = timezone.now(); schedule.next_run = timezone.now() + timedelta(minutes=schedule.interval_minutes)
...
def cloud_discovery_task(run_id: str):
    ...
    try:
        ...
        if provider == 'aws':
            try: import boto3
            except ImportError as exc: raise RuntimeError('AWS discovery requires boto3') from exc
            client = boto3.client('resourcegroupstaggingapi', region_name=(run.config or {}).get('region'))
            resources = client.get_resources().get('ResourceTagMappingList', [])
        elif provider == 'azure':
            try:
                from azure.identity import DefaultAzureCredential
                from azure.mgmt.resource import ResourceManagementClient
            except ImportError as exc: raise RuntimeError('Azure discovery requires azure-identity and azure-mgmt-resource') from exc
            ...
            try: from google.cloud import asset_v1
            except ImportError as exc: raise RuntimeError('GCP discovery requires google-cloud-asset') from exc
