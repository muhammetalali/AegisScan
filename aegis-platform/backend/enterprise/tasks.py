...
from .models import ContinuousAssuranceSchedule, Notification, ReportSchedule, CloudDiscoveryRun, ExternalIntegration, SBOMArtifact
...
def dispatch_integration(integration_id: str,event: dict):
    integration=ExternalIntegration.objects.get(pk=integration_id); return send_integration(integration,event)
...
def cloud_discovery_task(run_id: str):
    ...
    try:
        ...
        if provider=='aws':
            import boto3; client=boto3.client('resourcegroupstaggingapi',region_name=(run.config or {}).get('region')); resources=client.get_resources().get('ResourceTagMappingList',[])
        elif provider=='azure':
