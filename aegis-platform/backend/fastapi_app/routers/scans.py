...
class ScanCreate(BaseModel):
    ...
    # Kept for backwards-compatible request parsing. It is intentionally NOT
    # treated as an authorization grant; server-side Asset authorization is the
    # source of truth for network execution.
...
def _list_scans(user_id: str, project_id: Optional[str], status: Optional[str], limit: int, offset: int):
    qs = (
        Scan.objects.select_related('project').filter(project__members=user_id)
        | Scan.objects.select_related('project').filter(project__owner_id=user_id)
...
def _create_scan(scan: ScanCreate, user_id: str):
    with transaction.atomic():
        project = (
            Project.objects.select_for_update().filter(id=scan.project_id).filter(members=user_id).first()
            or Project.objects.select_for_update().filter(id=scan.project_id, owner_id=user_id).first()
        )
        if not project:
        ...
        if len(engines) > 1:
            raise HTTPException(
                status_code=400,
                detail='A Scan currently executes exactly one real engine; create one Scan per engine to preserve execution isolation.',
            )
        ...
        # Network execution must reference an already-persisted asset whose
        # authorization is explicit in server-side state. A request-body
        # `authorized=true` can never create authority or create an authorized
        # asset. This prevents client-controlled authorization elevation.
        if scan.scan_type in NETWORK_SCAN_TYPES:
            ...
            requested_target = (
                scan.config.get('target')
                or scan.config.get('host')
                or scan.config.get('ip')
                or scan.config.get('url')
            )
            if requested_target:
                normalized_requested = str(requested_target).strip()
                persisted_target = (
                    asset_config.get('url')
                ...
                if persisted_target != normalized_requested:
                    raise HTTPException(
                        status_code=409,
                        detail='Requested scan target does not match the authorized asset identity',
                    )

        if engines[0] == 'nuclei' and (not asset or not (asset.configuration or {}).get('url')):
...
def _get_scan(scan_id: str, user_id: str):
    return (
        Scan.objects.select_related('project').filter(id=scan_id).filter(project__members=user_id).first()
...
def _delete_scan(scan_id: str, user_id: str):
    scan = (
        Scan.objects.filter(id=scan_id, project__members=user_id).first()
...
def _logs(scan_id: str, user_id: str, limit: int):
    ...
    if not scan:
        return None
    return [
        {'id': str(x.id), 'level': x.level, 'message': x.message, 'context': x.context, 'created_at': x.created_at.isoformat()}
        for x in scan.logs.all()[:limit]
    ]
    ...
    return [
        {
            ...
            'status': x.status,
            'progress': x.progress,
            'findings_found': x.findings_found,
            'evidences_collected': x.evidences_collected,
