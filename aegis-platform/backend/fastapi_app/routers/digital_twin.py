...
def _scenario_response_sync(item: TwinScenario) -> ScenarioResponse:
    parameters = item.parameters or {}
    baseline = float(item.baseline_risk or 0.0)
    predicted = float(item.predicted_risk or 0.0)
    return ScenarioResponse(
        id=str(item.id), twin_id=str(item.twin_id), name=item.name, change_type=item.change_type,
        description=item.description, affected_nodes=item.affected_nodes or [], security_impact=predicted,
        performance_impact=float(parameters.get('performance_impact', 0.0) or 0.0),
        risk_reduction=max(0.0, baseline - predicted), recommendation=item.recommendation or '',
        status=item.status, created_at=item.created_at.isoformat(), source='postgresql'
    )


@sync_to_async
def _scenario_response(item: TwinScenario) -> ScenarioResponse:
    return _scenario_response_sync(item)
...
async def create_scenario(twin_id: UUID, scenario: ScenarioCreate, user=Depends(get_current_user)):
    ...
    item = await sync_to_async(TwinScenario.objects.create)(twin=twin, name=scenario.name, change_type=scenario.change_type, description=scenario.description, affected_nodes=scenario.affected_nodes, parameters=parameters, created_by_id=str(user.get('user_id')))
...
async def simulate_scenario(scenario_id: UUID, user=Depends(get_current_user)):
    ...
    return ScenarioSimulationResponse(
        scenario_id=str(item.id), status='queued', deterministic=True, source='postgresql',
        pre_change_risk=max(0.0, baseline), post_change_risk=max(0.0, predicted),
        risk_reduction=max(0.0, baseline - predicted), affected_nodes=item.affected_nodes or [],
        recommendation=item.recommendation or f'Simulation queued as task {task.id}.',
    )
...
async def check_drift(twin_id: UUID, current_assets: List[dict], user=Depends(get_current_user)):
    ...
    missing = sorted(observed - modeled); extra = sorted(modeled - observed)
