...
from django_project.users.models import APIKey, LoginAttempt, Team, User, UserRole, UserSession
...
def _audit_logs(user_id: str, limit: int, action: Optional[str], actor: Optional[str]):
    ...
    return [
        {
            'id': str(item.id),
            'user': item.user.email if item.user else None,
            'action': item.action,
            'result': item.result,
            'resource_type': item.resource_type,
            'resource_id': item.resource_id,
            'resource_repr': item.resource_repr,
            ...
            'timestamp': item.created_at.astimezone(timezone.utc).isoformat(),
        }
...
def _users(user_id: str):
    ...
    return [
        {'id': str(item.id), 'email': item.email, 'name': item.get_full_name(), 'role': item.role, 'status': 'active' if item.is_active else 'inactive', 'last_login': item.last_login.isoformat() if item.last_login else None}
        for item in qs
    ]
...
def _teams(user_id: str):
    ...
    return [
        {'id': str(team.id), 'name': team.name, 'description': team.description, 'members': team.members.count(), 'owner_id': str(team.owner_id), 'is_active': team.is_active}
        for team in qs
    ]
...
def _api_keys(user_id: str):
    ...
    return [
        {'id': str(item.id), 'name': item.name, 'prefix': item.key_prefix, 'permissions': item.permissions, 'created_at': item.created_at.astimezone(timezone.utc).isoformat(), 'last_used': item.last_used_at.astimezone(timezone.utc).isoformat() if item.last_used_at else None, 'expires_at': item.expires_at.astimezone(timezone.utc).isoformat() if item.expires_at else None, 'is_active': item.is_active, 'user_id': str(item.user_id), 'team_id': str(item.team_id) if item.team_id else None}
        for item in qs
    ]
...
def _sessions(user_id: str):
    ...
    if not current.is_staff:
        qs = qs.filter(user=current)
    return [
        {'id': str(item.id), 'user': item.user.email, 'ip': item.ip_address, 'user_agent': item.user_agent, 'location': item.location, 'is_current': item.is_current, 'created_at': item.created_at.astimezone(timezone.utc).isoformat(), 'expires_at': item.expires_at.astimezone(timezone.utc).isoformat(), 'last_activity': item.last_activity.astimezone(timezone.utc).isoformat()}
        for item in qs
    ]
...
def _login_attempts(user_id: str, limit: int):
    ...
    return [
        {'id': str(item.id), 'email': item.email, 'ip': item.ip_address, 'user_agent': item.user_agent, 'success': item.success, 'failure_reason': item.failure_reason, 'timestamp': item.created_at.astimezone(timezone.utc).isoformat()}
        for item in qs[:limit]
    ]
