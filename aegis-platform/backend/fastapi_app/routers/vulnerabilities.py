...
def _serialize(vulnerability: Vulnerability) -> VulnerabilityResponse:
    ...
    return VulnerabilityResponse(
        id=str(vulnerability.id), scan_id=str(vulnerability.scan_id), project_id=str(vulnerability.project_id),
        ...
        source_engine=source_engine, title=vulnerability.title, description=vulnerability.description,
        severity=vulnerability.severity, status=vulnerability.status, confidence=vulnerability.confidence,
        cvss_score=vulnerability.cvss_score, risk_score=vulnerability.risk_score,
...
async def list_vulnerabilities(project_id: Optional[str] = None, scan_id: Optional[str] = None, severity: Optional[str] = None, status: Optional[str] = None, assigned_to: Optional[str] = None, search: Optional[str] = None, limit: int = Query(50, le=200), offset: int = Query(0, ge=0), user=Depends(get_current_user)):
...
def _update_vulnerability(vuln_id: UUID, user_id: str, update: VulnerabilityUpdate):
    ...
    if update.assigned_to is not None:
        ...
        assignee = get_user_model().objects.filter(pk=update.assigned_to, is_active=True).first()
        if not assignee: raise ValueError('assigned_to user not found')
        vulnerability.assigned_to = assignee; vulnerability.assigned_at = datetime.now(timezone.utc)
    if update.remediation is not None: vulnerability.remediation = update.remediation
...
async def add_note(vuln_id: UUID, content: str, is_private: bool = False, user=Depends(get_current_user)):
    if not content.strip(): raise HTTPException(status_code=400, detail='content is required')
    note = await _add_note(vuln_id, str(user.get('user_id')), content.strip(), is_private)
    if not note: raise HTTPException(status_code=404, detail='Vulnerability not found')
