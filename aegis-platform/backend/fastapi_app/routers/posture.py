...
def _posture(project_id: str, user_id: str):
    ...
    recommendations = []
    if any(f.severity in {Vulnerability.Severity.CRITICAL, Vulnerability.Severity.HIGH} for f in active):
        recommendations.append('Remediate open critical and high severity findings.')
    if evidence_quality < 100:
        recommendations.append('Collect evidence for findings that have no evidence records.')
    if coverage < 100:
        recommendations.append('Run authorized assessments for assets without completed scans.')
    if assessments.exists() and controls < 100:
        recommendations.append('Address partial and non-compliant controls.')
...
def _history(project_id: str, user_id: str, limit: int):
    ...
    if not project:
        raise HTTPException(status_code=404, detail='Project not found or inaccessible')
    rows = []
    scans = Scan.objects.filter(project=project, status=Scan.Status.COMPLETED).order_by('-completed_at', '-created_at')[:limit]
    for scan in scans:
        findings = list(Vulnerability.objects.filter(scan=scan).only('severity', 'status'))
        timestamp = scan.completed_at or scan.created_at
        rows.append({'scan_id': str(scan.id), 'timestamp': timestamp.astimezone(timezone.utc).isoformat(), 'overall_score': _score_for_findings(findings), 'rating': _rating(_score_for_findings(findings))})
