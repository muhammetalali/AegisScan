...
def test_create_build_and_read_real_twin():
    user, project, asset_a, asset_b = _context()
    ...
    fetched = client.get(f'/digital-twin/twins/{twin_id}')
...
def test_scenario_is_persisted_and_unknown_node_is_rejected():
    user, project, asset_a, asset_b = _context()
    client = TestClient(app)
    twin_payload = client.post(f'/digital-twin/projects/{project.id}/twins', params={'name': 'scenario-model'}).json()
    twin_id = twin_payload['id']

    scenario = client.post(f'/digital-twin/twins/{twin_id}/scenarios', json={
        'name': 'isolate database',
        'change_type': 'network_isolation',
        'affected_nodes': [str(asset_b.id)],
        'parameters': {'reason': 'controlled test'},
    })
    ...
    bad = client.post(f'/digital-twin/twins/{twin_id}/scenarios', json={
        'name': 'invalid',
        'change_type': 'unknown',
        'affected_nodes': ['00000000-0000-0000-0000-000000000000'],
    })
