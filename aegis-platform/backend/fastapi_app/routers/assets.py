...
def _get_asset(asset_id: str, user_id: str):
    ...
    if owner_asset: return owner_asset
    ...
    with transaction.atomic():
        ...
        if "configuration" in data and data["configuration"] is not None:
            ...
            if latest_authorization and latest_authorization.authorized:
                ...
                AssetAuthorization.objects.create(asset=asset, actor_id=user_id, authorized=False, target_snapshot=previous_target[:500], reason="Authorization revoked because asset configuration changed", supersedes=latest_authorization)
...
@sync_to_async
def _set_asset_authorization(asset_id: str, user_id: str, authorized: bool, reason: str, is_staff: bool, correlation_id: Optional[UUID], expires_at):
    ...
    with transaction.atomic():
        ...
        configuration = dict(asset.configuration or {}); configuration["authorized"] = authorized; asset.configuration = configuration
        asset.save(update_fields=["configuration", "updated_at"])
        AssetAuthorization.objects.create(asset=asset, actor_id=user_id, authorized=authorized, target_snapshot=target[:500], reason=reason, correlation_id=correlation_id or UUID(int=0), supersedes=latest, expires_at=expiry) if correlation_id else AssetAuthorization.objects.create(asset=asset, actor_id=user_id, authorized=authorized, target_snapshot=target[:500], reason=reason, supersedes=latest, expires_at=expiry)
        return asset
...
async def set_asset_authorization(asset_id: str, update: AssetAuthorizationUpdate, user=Depends(get_current_user)):
    return _asset_response(await _set_asset_authorization(asset_id, str(user.get("user_id")), update.authorized, update.reason, bool(user.get("is_staff")), update.correlation_id, update.expires_at))
