...
def _load_graph(project_id: str, user_id: str) -> AttackPathGraph:
    ...
    nodes = [AttackPathNode(id=str(asset.id), name=asset.name, kind=str(asset.type), criticality=str(asset.criticality), open_finding_weight=round(finding_weight.get(str(asset.id), 0.0), 2), internet_exposed=_asset_exposure(asset.configuration or {})) for asset in assets]
    ...
    edges = [AttackPathEdge(source=str(r.source_id), target=str(r.target_id), relationship=str(r.relationship_type), metadata=r.metadata or {}) for r in relationships]
...
def _analyze(request: AttackPathAnalysisRequest, project_id: str, user_id: str) -> AttackPathAnalysisResponse:
    ...
    if paths:
        ...
        for path in paths:
            ...
            row, _ = AttackPath.objects.get_or_create(
