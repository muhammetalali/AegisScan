...
def _temp_file(data: bytes, suffix: str, cleanup: list[str]) -> str:
    fd, path = tempfile.mkstemp(prefix='aegis-kube-material-', suffix=suffix)
    os.fchmod(fd, 0o600)
...
def load_kubeconfig(path: str, authorized_target: str) -> KubernetesTransport:
    ...
    server = canonical_server(str(cluster.get('server') or ''))
    if server != canonical_server(authorized_target):
    ...
    if cluster.get('tls-server-name'):
        raise KubernetesSecurityError('kubeconfig tls-server-name overrides are forbidden')
    if 'certificate-authority' in cluster:
        raise KubernetesSecurityError(
            'external certificate-authority paths are forbidden; embed certificate-authority-data'
        )

    forbidden_user_fields = {
        'exec', 'auth-provider', 'tokenFile', 'client-certificate', 'client-key', 'username', 'password'
    }
    present_forbidden = sorted(
        field for field in forbidden_user_fields if user.get(field) not in (None, '', {})
    )
    if present_forbidden:
        raise KubernetesSecurityError(
            'kubeconfig user contains unsupported external or executable authentication fields: '
            f'{present_forbidden}'
        )
    ...
    try:
        ...
        if token and has_client_pair:
            raise KubernetesSecurityError(
                'kubeconfig must use one authentication mechanism: token or embedded client certificate'
            )
        if not token and not has_client_pair:
            raise KubernetesSecurityError(
                'kubeconfig must contain a token or embedded client certificate credentials'
            )
        if token and (len(token) > 16384 or any(ch in token for ch in '\r\n\x00')):
            raise KubernetesSecurityError('kubeconfig token is invalid')
        if has_client_pair and not (client_cert_data and client_key_data):
            raise KubernetesSecurityError(
...
def _finding(
    ...
    *,
    ...
    location = '/'.join(
        part for part in (namespace or '_cluster', resource_kind, resource_name, container) if part
    )
    return {
...
def workload_findings(kind: str, resource: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for field, label in (('hostNetwork', 'network'), ('hostPID', 'PID'), ('hostIPC', 'IPC')):
        if spec.get(field) is True:
            findings.append(_finding(
                ...
                resource_name=name,
                cwe_id='CWE-250',
                owasp_category='Kubernetes Security - Workload Isolation',
            ))
    ...
    for container_obj in containers[:1000]:
        ...
        security = (
            container_obj.get('securityContext')
            if isinstance(container_obj.get('securityContext'), dict)
            else {}
        )
        if security.get('privileged') is True:
            findings.append(_finding(
                ...
                cwe_id='CWE-250',
                owasp_category='Kubernetes Security - Privilege Escalation',
            ))
        if security.get('allowPrivilegeEscalation') is True:
            findings.append(_finding(
                'kubernetes.workload.allow-privilege-escalation',
                'Container explicitly allows privilege escalation',
                'medium',
        ...
        if any(str(value).upper() == 'ALL' for value in added):
            findings.append(_finding(
                ...
                'Container requests all Linux capabilities',
                'high',
        ...
        if run_as_user == 0:
            findings.append(_finding(
                ...
                'Container is explicitly configured to run as UID 0',
                'medium',
...
def _role_is_wildcard(role: dict[str, Any]) -> bool:
    ...
    for rule in rules:
        if not isinstance(rule, dict):
            continue
        verbs = (
            {str(value) for value in rule.get('verbs', []) if isinstance(value, str)}
            if isinstance(rule.get('verbs'), list)
            else set()
        )
        resources = (
            {str(value) for value in rule.get('resources', []) if isinstance(value, str)}
            if isinstance(rule.get('resources'), list)
            else set()
        )
...
def rbac_findings(
    roles: list[dict[str, Any]], bindings: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    for binding in bindings:
        ...
        if role_kind == 'ClusterRole' and role_name in dangerous:
            findings.append(_finding(
                'kubernetes.rbac.bound-wildcard-clusterrole',
                'Bound ClusterRole grants wildcard resource and verb access',
                'high',
                ...
                resource_name=name,
                cwe_id='CWE-732',
                owasp_category='Kubernetes Security - Excessive RBAC',
            ))
            ...
            for subject in subjects[:500]:
                ...
                if (
                    ...
                    findings.append(_finding(
                        ...
                        'Default ServiceAccount is bound to cluster-admin',
                        'critical',
...
def analyze_resources(
    *,
    ...
    findings: list[dict[str, Any]] = []
    for pod in pods:
        findings.extend(workload_findings('Pod', pod))
    for deployment in deployments:
        findings.extend(workload_findings('Deployment', deployment))
    findings.extend(rbac_findings(roles, bindings))
    ...
    summary = {
        ...
        'read_only': True,
        'requested_paths': list(_READ_PATHS),
...
def analyze_cluster(target: str, kubeconfig_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        return analyze_resources(
            ...
            deployments=_items(deployments_doc),
            roles=_items(roles_doc),
            bindings=_items(bindings_doc),
            coverage=coverage,
        )
    finally:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description='AegisScan bounded read-only Kubernetes security posture analyzer'
    )
