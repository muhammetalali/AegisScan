...
def require_bound_scan_authorization(scan_id: str) -> tuple[Scan | None, str, AssetAuthorization | None]:
    ...
    target allowlist and worker-side DNS destination policy.
    """
    with transaction.atomic():
        ...
        if scan.scan_type in _NETWORK_SCAN_TYPES:
            ok, reason = _require_worker_egress(authorized_target, url=scan.scan_type == Scan.Type.URL)
            if not ok:
...
def revalidate_bound_authorization(scan: Scan, decision: AssetAuthorization) -> tuple[bool, str]:
    """Re-check authorization and DNS destination immediately before evidence commit."""
    with transaction.atomic():
        asset = Asset.objects.select_for_update().get(pk=scan.asset_id)
