...
class IntelligenceFusion:
    ...
    def enrich_cve(self, cve_id: str, *, nvd_api_key: Optional[str] = None) -> FusionResult:
        ...
        failures: list[str] = []
        try:
            result = self.nvd.fetch_cve(cve, api_key=nvd_api_key); sources['nvd'] = result.data; source_urls['nvd'] = result.url
        except IntelligenceProviderError as exc: failures.append(f'nvd:{exc}')
        try:
            result = self.osv.fetch_vulnerability(cve); sources['osv'] = result.data; source_urls['osv'] = result.url
        ...
        except IntelligenceProviderError as exc: failures.append(f'epss:{exc}')
        if not sources: raise IntelligenceFusionError('No intelligence provider returned data: ' + '; '.join(failures))

        epss_value = None
        epss_rows = sources.get('epss', {}).get('data', [])
        if epss_rows and isinstance(epss_rows[0], dict):
            try: epss_value = float(epss_rows[0].get('epss'))
            except (TypeError, ValueError): epss_value = None
        kev_known = bool(sources.get('cisa_kev', {}).get('known_exploited'))
        confidence = 40.0 + (15.0 if 'nvd' in sources else 0.0) + (15.0 if 'osv' in sources else 0.0) + (20.0 if kev_known else 0.0) + (10.0 if epss_value is not None else 0.0)
        ...
        if kev_known: recommendation = 'Prioritize remediation: CISA KEV identifies this CVE as known exploited.'
        elif epss_value is not None and epss_value >= 0.5: recommendation = 'Prioritize investigation: EPSS indicates elevated exploitation probability.'
        explanation = 'Sources successfully queried: ' + ', '.join(sorted(sources))
        if failures: explanation += '. Provider failures: ' + '; '.join(failures)
        if conflicts: explanation += ' Conflicts detected: ' + ' '.join(conflicts)
    ...
    def persist(result: FusionResult, *, actor_id: str | None = None) -> IntelligenceEnrichment:
        return IntelligenceEnrichment.objects.create(
            cve_id=result.cve_id, sources=result.sources, source_urls=result.source_urls,
            provider_failures=result.provider_failures, confidence=result.confidence,
            conflicts=result.conflicts, recommendation=result.recommendation,
            explanation=result.explanation, observed_at=datetime.now(timezone.utc), observed_by_id=actor_id,
        )
