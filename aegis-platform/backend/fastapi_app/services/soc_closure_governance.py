...
def _remediation_proof(*, finding_id: str, validation_id: str, project_id: str, state: InvestigationCaseState) -> tuple[ValidationRun, Evidence, str]:
    ...
    if validation.finding.project_id != state.case.project_id or str(validation.finding.project_id) != str(project_id):
    ...
    source_sha = _sha({
        'validation_id': str(validation.id), 'validation_status': validation.status,
        'remediation_state': get_state(validation), 'finding_present': result.get('finding_present'),
        'evidence_id': str(evidence.id), 'evidence_sha256': evidence.sha256,
        'authorization_decision_id': str(validation.authorization_decision_id or ''),
    })
    return validation, evidence, source_sha


def _disposition_proof(*, finding_id: str, disposition_id: str, closure_type: str, organization_id: str) -> tuple[FindingDisposition, str]:
    ...
    if closure_type in {FindingDisposition.Disposition.ACCEPTED_RISK, FindingDisposition.Disposition.WONT_FIX}:
        if disposition.review_at is None or disposition.review_at <= timezone.now():
            raise ClosureGovernanceError('Risk disposition is expired and cannot authorize closure.')
    source_sha = _sha({
        'disposition_id': str(disposition.id), 'disposition': disposition.disposition,
        'policy_version': disposition.policy_version, 'request_fingerprint': disposition.request_fingerprint,
        'risk_correlation_sha256': disposition.risk_correlation_sha256,
        'review_at': disposition.review_at.isoformat() if disposition.review_at else '',
...
def close_investigation_case(
    *, case_id: str, project_id: str, user_id: str, expected_version: int,
    finding_id: str, closure_type: str, rationale: str = '',
    validation_id: str | None = None, disposition_id: str | None = None,
) -> ClosureResult:
    ...
    with transaction.atomic():
        state = (
            InvestigationCaseState.objects.select_for_update().select_related('case', 'decision_action')
        ...
        if closure_type == InvestigationClosure.ClosureType.REMEDIATED:
            ...
            validation, evidence, source_sha = _remediation_proof(
                finding_id=finding_id, validation_id=validation_id, project_id=project_id, state=state,
            )
            source_id = str(validation.id)
        else:
            if not disposition_id or validation_id:
                raise ClosureGovernanceError('Disposition closure requires disposition_id only.')
            disposition, source_sha = _disposition_proof(
                finding_id=finding_id, disposition_id=disposition_id, closure_type=closure_type,
                organization_id=str(link.organization_id),
            )
            source_id = str(disposition.id)

        fingerprint = _sha({
            'case_id': str(case_id), 'generation': state.generation, 'finding_id': str(finding_id),
            'closure_type': closure_type, 'source_id': source_id, 'source_sha256': source_sha,
            'decision_action_id': str(state.decision_action_id or ''), 'policy_version': POLICY_VERSION,
            'rationale': normalized_rationale,
        })
        if existing is not None:
            if existing.closure_fingerprint == fingerprint:
        ...
        if validation is not None:
            remediation_transition(
                validation.id, RemediationState.CLOSED,
                reason=f'Governed SOC investigation closure {case.id}; {normalized_rationale}'.strip(),
                evidence_id=str(evidence.id),
            )

        closure = InvestigationClosure.objects.create(
            case=case, finding_id=finding_id, closure_type=closure_type,
            validation_run=validation, disposition=disposition, evidence=evidence,
            decision_action=state.decision_action, policy_version=POLICY_VERSION,
            source_sha256=source_sha, closure_fingerprint=fingerprint,
            rationale=normalized_rationale, closed_by_id=user_id,
        )
        ...
        _append_event_locked(case, user_id, 'case.closed.governed', {
            'closure_id': str(closure.id), 'closure_type': closure_type,
            'finding_id': str(finding_id), 'source_id': source_id,
            'source_sha256': source_sha, 'closure_fingerprint': fingerprint,
            'policy_version': POLICY_VERSION, 'old_status': old_status,
            'new_status': InvestigationCase.Status.CLOSED, 'version': state.version,
        })
...
def get_investigation_closure(*, case_id: str, project_id: str, user_id: str) -> dict[str, Any]:
    ...
    return {
        'closure_id': str(closure.id), 'case_id': str(closure.case_id),
        'finding_id': str(closure.finding_id), 'closure_type': closure.closure_type,
        'validation_id': str(closure.validation_run_id) if closure.validation_run_id else None,
        'disposition_id': str(closure.disposition_id) if closure.disposition_id else None,
        'evidence_id': str(closure.evidence_id) if closure.evidence_id else None,
        'decision_action_id': closure.decision_action_id, 'source_sha256': closure.source_sha256,
        'closure_fingerprint': closure.closure_fingerprint, 'policy_version': closure.policy_version,
        'closed_by': str(closure.closed_by_id), 'closed_at': closure.closed_at.astimezone(dt_timezone.utc).isoformat(),
    }
