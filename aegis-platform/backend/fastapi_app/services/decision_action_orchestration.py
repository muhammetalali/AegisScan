...
def _hydrate(cur, row: tuple[Any, ...]) -> dict[str, Any]:
    action_id, decision_id, node_id, title, owner, requested_by, sla_hours, state, risk_before, confidence_before, priority, recommended_action, remediation_plan, created_at, updated_at, version = row
        ...
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO security_decision_actions (
                    ...
                    risk_before, confidence_before, priority, recommended_action, remediation_plan, created_at, updated_at
                ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            """, (
                action_id, str(decision.get("decisionId") or ""), node_id,
                f"Remediate: {decision.get('label', 'Security finding')}", owner, requested_by,
                max(1, sla_hours), "pending", int(decision.get("risk", 0) or 0), int(decision.get("confidence", 0) or 0),
                int(decision.get("priority", 0) or 0), decision.get("recommendedAction", "Apply remediation and re-validate."),
                json.dumps(decision.get("revalidationPlan", [])), now, now,
            ))
...
def transition(action_id: str, state: str, actor: str, note: str | None = None) -> dict[str, Any]:
    ...
    try:
        with conn.cursor() as cur:
            ...
            now = _now()
            cur.execute("UPDATE security_decision_actions SET state=%s, updated_at=%s, version=version+1 WHERE action_id=%s AND version=%s", (state, now, action_id, version))
            if cur.rowcount != 1:
