...
async def lifespan(app: FastAPI):
    ...
    yield
    await workflow_bridge.stop(); await scan_orchestrator.stop(); logger.info('Shutting down AegisScan FastAPI server...')
...
async def websocket_scan_progress(websocket: WebSocket, scan_id: str):
    ...
    try:
        ...
        websocket_manager.disconnect(scan_id, websocket); websocket_manager.disconnect(f'scan_{scan_id}', websocket)
...
app.include_router(scans.router, prefix='/scans', tags=['Scans']); app.include_router(vulnerabilities.router, prefix='/vulnerabilities', tags=['Vulnerabilities']); app.include_router(reports.router, prefix='/reports', tags=['Reports']); app.include_router(assets.router, prefix='/assets', tags=['Assets']); app.include_router(assets.router, prefix='/api/v1/assets', tags=['Assets']); app.include_router(compliance.router, prefix='/compliance', tags=['Compliance']); app.include_router(knowledge.router, prefix='/knowledge', tags=['Knowledge']); app.include_router(digital_twin.router, prefix='/digital-twin', tags=['Digital Twin']); app.include_router(posture.router, prefix='/posture', tags=['Security Posture']); app.include_router(system.router, prefix='/system', tags=['System'])
...
async def enable_engine(engine_name: str, user=Depends(get_current_user)): return await scan_orchestrator.enable_engine(engine_name)
@app.post('/engines/{engine_name}/disable')
async def disable_engine(engine_name: str, user=Depends(get_current_user)): return await scan_orchestrator.disable_engine(engine_name)
