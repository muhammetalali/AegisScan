...
from .routers import scans, vulnerabilities, remediation, reports, assets, compliance, knowledge, digital_twin, posture, system, dashboard, validations, audit, assurance, assurance_graph, security_decision, decision_actions, governance, policy, enterprise
...
async def _dependency_readiness()->dict:
    def check_dependencies():
        ...
        try:
            with db_conn.cursor() as cursor: cursor.execute('SELECT 1'); cursor.fetchone()
        finally: db_conn.close()
        redis_client=redis.from_url(settings.REDIS_URL,socket_connect_timeout=3,socket_timeout=3)
        try:
            if redis_client.ping() is not True: raise RuntimeError('Redis ping returned false')
