...
def _ingest_nuclei_findings(scan: Scan, evidence: Evidence, raw_output: str) -> list[Vulnerability]:
    ...
    for item in _parse_nuclei_findings(raw_output):
        vulnerability = Vulnerability.objects.filter(
            scan=scan, asset=scan.asset, source_engine='nuclei',
            title=item['title'], url=item['url'][:500],
        ).first()
        if vulnerability is None:
            vulnerability = Vulnerability.objects.create(
                ...
                category='web', cwe_id=item['cwe_id'], cve_ids=item['cve_ids'],
                tags=[x for x in [item['template_id'], item['matcher_name']] if x],
                url=item['url'][:500], method=item['method'][:10],
                risk_score=_risk_score(item['severity']), evidence_count=0,
                verified_evidence_count=0, validation_status='unverified',
                remediation=item['remediation'], references=item['references'],
                source_engine='nuclei', raw_data=item['record'],
            )
        else:
        ...
        if evidence.finding_id in {None, vulnerability.id}:
            ...
            Evidence.objects.get_or_create(
                ...
                source='nuclei', evidence_type='scanner_output',
                defaults={'raw_output': evidence.raw_output, 'metadata': {**(evidence.metadata or {}), 'finding_id': str(vulnerability.id)}},
            )
...
def _fail_scan(scan: Scan, execution: ScanEngineExecution, message: str) -> dict[str, Any]:
    ...
    ScanLog.objects.create(scan=scan, engine_execution=execution, level=ScanLog.Level.ERROR, message='scanner execution failed', context={'error': message})
...
def run_nmap_scan(self, scan_id: str) -> dict[str, Any]:
    ...
    if configuration.get('authorized') is not True:
        ...
        return _fail_scan(scan, _start_execution(scan,engine), 'Execution blocked: asset is not explicitly marked authorized.')
    ...
    if not is_target_authorized(target):
        engine = _ensure_engine('nmap','Nmap',ScanEngine.EngineCategory.RECON,300)
        return _fail_scan(scan, _start_execution(scan,engine), 'Execution blocked: target is outside the server-side authorized scan scope.')
    scan.status=Scan.Status.RUNNING; scan.started_at=datetime.now(timezone.utc); scan.current_phase='nmap'; scan.current_engine='nmap'; scan.progress=10
    scan.save(update_fields=['status','started_at','current_phase','current_engine','progress','updated_at'])
    execution = _start_execution(scan,_ensure_engine('nmap','Nmap',ScanEngine.EngineCategory.RECON,300))
    try:
        result = get_tool('nmap').run(ToolRequest(target=target, authorized=True), timeout=120 if scan.depth == Scan.Depth.QUICK else 300)
        ...
        with transaction.atomic():
            ...
            execution.progress=100; execution.completed_at=completed_at; execution.findings_found=len(findings); execution.evidences_collected=1
            ...
            execution.save(update_fields=['status','progress','completed_at','findings_found','evidences_collected','result_data','logs','updated_at'])
            scan.status=Scan.Status.COMPLETED if result.exit_code==0 else Scan.Status.PARTIAL; scan.progress=100; scan.completed_at=completed_at; scan.findings_count=len(findings)
            scan.engine_results={**(scan.engine_results or {}),'nmap':execution.result_data}
...
def run_nuclei_scan(self, scan_id: str) -> dict[str, Any]:
    ...
    if configuration.get('authorized') is not True:
        return _fail_scan(scan,execution,'Execution blocked: asset is not explicitly marked authorized.')
    if not target:
        return _fail_scan(scan,execution,'Nuclei requires an authorized http/https URL target')
    if not is_target_authorized(target):
        return _fail_scan(scan,execution,'Execution blocked: target is outside the server-side authorized scan scope.')
    scan.status=Scan.Status.RUNNING; scan.started_at=datetime.now(timezone.utc); scan.current_phase='nuclei'; scan.current_engine='nuclei'; scan.progress=10
    scan.save(update_fields=['status','started_at','current_phase','current_engine','progress','updated_at'])
    try:
        result=run_nuclei(target,timeout=600)
        parsed=_parse_nuclei_findings(result.stdout)
        completed_at=datetime.now(timezone.utc)
        with transaction.atomic():
            ...
            execution.status=ScanEngineExecution.ExecutionStatus.COMPLETED if result.exit_code==0 else ScanEngineExecution.ExecutionStatus.FAILED
            execution.progress=100; execution.completed_at=completed_at; execution.findings_found=len(findings); execution.evidences_collected=1
            execution.result_data={'tool':result.tool,'target':result.target,'exit_code':result.exit_code,'result_count':len(parsed),'evidence_id':str(evidence.id),'finding_ids':[str(v.id) for v in findings]}
            execution.logs=result.stderr or ''
            execution.save(update_fields=['status','progress','completed_at','findings_found','evidences_collected','result_data','logs','updated_at'])
            scan.status=Scan.Status.COMPLETED if result.exit_code==0 else Scan.Status.PARTIAL; scan.progress=100; scan.completed_at=completed_at
