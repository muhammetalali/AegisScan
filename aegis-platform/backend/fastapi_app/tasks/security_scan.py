...
def _cancelled_scan(scan: Scan, execution: ScanEngineExecution, message: str)->dict[str,Any]:
    now=datetime.now(timezone.utc)
    ...
    checkpoint_scan(scan,state=Scan.Status.CANCELLED,metadata={'reason':message})
    return {'status':'cancelled','scan_id':str(scan.id),'error':message}
