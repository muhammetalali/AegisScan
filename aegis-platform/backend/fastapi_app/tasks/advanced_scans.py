...
from fastapi_app.services.scanner_adapters import run_masscan, run_semgrep, validate_authorized_target, validate_code_target
...
def _ensure_engine(name: str, display_name: str, category: str, timeout: int) -> ScanEngine:
    engine, _ = ScanEngine.objects.get_or_create(
        name=name,
        defaults={'display_name': display_name, 'description': f'{display_name} scanner engine', 'category': category, 'version': '1.0.0', 'status': ScanEngine.EngineStatus.ACTIVE, 'is_core': True, 'timeout': timeout},
    )
    return engine
...
def _execution(scan: Scan, engine: ScanEngine) -> ScanEngineExecution:
    ...
    item.status=ScanEngineExecution.ExecutionStatus.RUNNING
    item.progress=10
...
def _finish_failed(scan: Scan, execution: ScanEngineExecution, message: str):
    ...
    execution.status=ScanEngineExecution.ExecutionStatus.FAILED; execution.progress=100; execution.completed_at=now; execution.error_message=message
    execution.save(update_fields=['status','progress','completed_at','error_message','updated_at'])
    ScanLog.objects.create(scan=scan,engine_execution=execution,level=ScanLog.Level.ERROR,message='advanced scanner execution failed',context={'error':message})
    scan.status=Scan.Status.FAILED; scan.error_message=message; scan.completed_at=now
...
def _masscan_findings(raw: str) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        for port in record.get('ports',[]) if isinstance(record.get('ports'),list) else []:
            ...
            out.append({'ip':ip,'port':port_no,'protocol':protocol,'record':record})
    return out
...
def _semgrep_findings(raw: str) -> list[dict[str, Any]]:
    ...
    for item in results:
        ...
        sev={'error':Vulnerability.Severity.HIGH,'warning':Vulnerability.Severity.MEDIUM,'info':Vulnerability.Severity.INFO}.get(severity,Vulnerability.Severity.MEDIUM)
...
def _git_checkout(asset_config: dict[str,Any]) -> tuple[str, tempfile.TemporaryDirectory[str]]:
    repo_url=str(asset_config.get('repo_url') or '').strip()
    if not repo_url.startswith(('https://','ssh://','git@')):
        raise ValueError('Semgrep repository asset requires a git repository URL')
    workdir=tempfile.TemporaryDirectory(prefix='aegis-semgrep-')
    destination=Path(workdir.name)/'repo'
    token_env=str(asset_config.get('token_env') or '').strip()
    env=os.environ.copy()
    if token_env and env.get(token_env):
        token=env[token_env]
        if repo_url.startswith('https://') and '@' not in repo_url.split('://',1)[1].split('/',1)[0]:
            repo_url='https://oauth2:%s@%s' % (token,repo_url.split('://',1)[1])
    branch=str(asset_config.get('branch') or '').strip()
    ...
    return str(destination),workdir
...
def run_masscan_scan(self, scan_id: str) -> dict[str,Any]:
    ...
    engine=_ensure_engine('masscan','Masscan',ScanEngine.EngineCategory.RECON,300)
    execution=_execution(scan,engine)
    target=config.get('host') or config.get('ip') or config.get('domain')
    ...
    try:
        result=run_masscan(str(target),ports=str(config.get('ports') or scan.config.get('ports') or '1-65535'),rate=int(config.get('rate') or scan.config.get('rate') or 1000),timeout=300)
        observations=_masscan_findings(result.stdout)
        now=datetime.now(timezone.utc)
        with transaction.atomic():
            evidence=Evidence.objects.create(scan=scan,asset=scan.asset,source='masscan',evidence_type='scanner_output',raw_output=result.stdout,metadata={'stderr':result.stderr,'exit_code':result.exit_code,'target':result.target,'format':'json'},collected_by=scan.initiated_by)
            findings=[]
            for obs in observations:
                ...
                if finding is None:
                    finding=Vulnerability.objects.create(scan=scan,project=scan.project,asset=scan.asset,title=f"Open {obs['protocol'].upper()} port {obs['port']}",description=f"Masscan detected an open {obs['protocol'].upper()} port {obs['port']} on {obs['ip'] or target}.",severity=Vulnerability.Severity.INFO,status=Vulnerability.Status.OPEN,confidence=Vulnerability.Confidence.HIGH,category='network',tags=['masscan',obs['protocol']],risk_score=10.0,validation_status='unverified',source_engine='masscan',raw_data=obs['record'])
                evidence.finding=finding; evidence.save(update_fields=['finding'])
                finding.evidence_count=finding.evidence_records.count(); finding.save(update_fields=['evidence_count','updated_at']); findings.append(finding)
            execution.status=ScanEngineExecution.ExecutionStatus.COMPLETED if result.exit_code==0 else ScanEngineExecution.ExecutionStatus.FAILED; execution.progress=100; execution.completed_at=now; execution.findings_found=len(findings); execution.evidences_collected=1; execution.result_data={'tool':'masscan','target':result.target,'exit_code':result.exit_code,'finding_ids':[str(v.id) for v in findings],'evidence_id':str(evidence.id)}; execution.save(update_fields=['status','progress','completed_at','findings_found','evidences_collected','result_data','updated_at'])
            scan.status=Scan.Status.COMPLETED if result.exit_code==0 else Scan.Status.PARTIAL; scan.progress=100; scan.completed_at=now; scan.findings_count=len(findings); scan.engine_results={**(scan.engine_results or {}),'masscan':execution.result_data}; scan.save(update_fields=['status','progress','completed_at','findings_count','engine_results','updated_at'])
        return {'status':scan.status,'scan_id':scan_id,'tool':'masscan','target':result.target,'finding_ids':[str(v.id) for v in findings]}
    except Exception as exc: return _finish_failed(scan,execution,str(exc))
...
def run_semgrep_scan(self, scan_id: str) -> dict[str,Any]:
    ...
    execution=_execution(scan,engine)
    if config.get('authorized') is not True: return _finish_failed(scan,execution,'Execution blocked: asset is not explicitly marked authorized.')
    workdir=None; holder=None
    try:
        if config.get('repo_url'):
            workdir,holder=_git_checkout(config)
            source=workdir
        else:
            source=validate_code_target(str(config.get('path') or scan.config.get('path') or ''))
        result=run_semgrep(source,timeout=900)
        observations=_semgrep_findings(result.stdout)
        now=datetime.now(timezone.utc)
        with transaction.atomic():
            evidence=Evidence.objects.create(scan=scan,asset=scan.asset,source='semgrep',evidence_type='scanner_output',raw_output=result.stdout,metadata={'stderr':result.stderr,'exit_code':result.exit_code,'source':source,'format':'json'},collected_by=scan.initiated_by)
            findings=[]
            for obs in observations:
                ...
                if finding is None:
                    finding=Vulnerability.objects.create(scan=scan,project=scan.project,asset=scan.asset,title=obs['message'][:300],description=obs['message'],severity=obs['severity'],status=Vulnerability.Status.OPEN,confidence=Vulnerability.Confidence.HIGH,category='code',file_path=obs['path'][:500],line_start=max(0,obs['line']) or None,risk_score={Vulnerability.Severity.HIGH:80.0,Vulnerability.Severity.MEDIUM:60.0,Vulnerability.Severity.INFO:10.0}[obs['severity']],validation_status='unverified',source_engine='semgrep',raw_data=obs['record'])
                Evidence.objects.create(scan=scan,asset=scan.asset,finding=finding,source='semgrep',evidence_type='scanner_output',raw_output=evidence.raw_output,metadata={'check_id':obs['check_id'],'path':obs['path'],'line':obs['line']},collected_by=scan.initiated_by)
                finding.evidence_count=finding.evidence_records.count(); finding.save(update_fields=['evidence_count','updated_at']); findings.append(finding)
            execution.status=ScanEngineExecution.ExecutionStatus.COMPLETED if result.exit_code==0 else ScanEngineExecution.ExecutionStatus.FAILED; execution.progress=100; execution.completed_at=now; execution.findings_found=len(findings); execution.evidences_collected=1; execution.result_data={'tool':'semgrep','source':source,'exit_code':result.exit_code,'finding_ids':[str(v.id) for v in findings],'evidence_id':str(evidence.id)}; execution.save(update_fields=['status','progress','completed_at','findings_found','evidences_collected','result_data','updated_at'])
            scan.status=Scan.Status.COMPLETED if result.exit_code==0 else Scan.Status.PARTIAL; scan.progress=100; scan.completed_at=now; scan.findings_count=len(findings); scan.engine_results={**(scan.engine_results or {}),'semgrep':execution.result_data}; scan.save(update_fields=['status','progress','completed_at','findings_count','engine_results','updated_at'])
