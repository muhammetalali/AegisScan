...
def test_nmap_redelivery_does_not_rerun_or_duplicate_durable_state(monkeypatch):
    ...
    monkeypatch.setattr(security_scan, 'get_tool', lambda name: stub)

    first = security_scan.run_nmap_scan.run(str(scan.id))
    second = security_scan.run_nmap_scan.run(str(scan.id))
    ...
    assert ScanLog.objects.filter(scan=scan, message='nmap execution started').count() == 1
