...
def _linux_privilege_boundary() -> dict[str, Any]:
    """Require the same minimal raw-socket capability used by production Nmap.

    The parity harness starts its container with uid 0 only so Docker can grant
    exactly CAP_NET_RAW after dropping every other capability. The provider is
    parity-only, is never packaged into the production profile, and fails
    closed unless no_new_privs is set and the effective/permitted/bounding
    capability sets contain exactly CAP_NET_RAW.
    """
    values: dict[str, str] = {}
    try:
