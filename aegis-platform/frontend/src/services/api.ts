...
const platformApi = axios.create({ baseURL: configuredApiUrl ? configuredApiUrl.replace(/\/api\/v1$/, '') : (import.meta.env.DEV ? 'http://localhost:8000' : ''), timeout: 10000, withCredentials: true })

api.interceptors.request.use((config: InternalAxiosRequestConfig) => config, (error) => Promise.reject(error))
let isRefreshing = false
let failedQueue: Array<{ resolve: (value: unknown) => void; reject: (reason: unknown) => void }> = []
const processQueue = (error: Error | null) => { failedQueue.forEach((prom) => error ? prom.reject(error) : prom.resolve(true)); failedQueue = [] }
...
export const apiHelpers = {
  ...
  health: <T>() => platformApi.get<T>('/health').then((r) => r.data),
}

export const uploadFile = async (file: File, onProgress?: (progress: number) => void) => {
  const formData = new FormData(); formData.append('file', file)
  const response = await api.post('/uploads/', formData, { headers: { 'Content-Type': 'multipart/form-data' }, onUploadProgress: (event) => { if (event.total && onProgress) onProgress(Math.round((event.loaded * 100) / event.total)) } })
...
export const createWebSocket = (url: string, protocols?: string | string[]) => {
  ...
  const defaultWsUrl = import.meta.env.DEV ? 'ws://localhost:8000' : `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}`
