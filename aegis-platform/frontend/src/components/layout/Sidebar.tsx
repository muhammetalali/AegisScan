    ...
    <div className="flex-1 overflow-y-auto px-2.5 py-4 space-y-5">
      {groups.map(group => (
        <section key={group.label}>
          {(open || isMobile) && <div className="px-2 mb-2 text-[9px] font-bold uppercase tracking-[0.2em] text-muted-foreground/80">{t(group.label)}</div>}
          ...
          <ul className="space-y-1">
            {group.items.map(item => {
              ...
              return <li key={item.href}>
                <NavLink to={item.href} onClick={() => isMobile && onMobileOpenChange(false)} title={!open && !isMobile ? t(item.name) : undefined}
                  className={cn('enterprise-nav group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-[12px] font-medium transition-all duration-200', active ? 'bg-primary/10 text-primary shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--primary)_14%,transparent),0_10px_25px_color-mix(in_srgb,var(--primary)_7%,transparent)]' : 'text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground', !open && !isMobile && 'justify-center px-2')}>
                  ...
                  {(open || isMobile) && <span className="truncate">{t(item.name)}</span>}
