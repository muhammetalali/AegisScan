...
function Metric({ item }: { item: CisoTimelineMetric }) {
  const delta = item.current - item.before
  const improved = item.inverse ? delta > 0 : delta < 0
  return <div className="rounded-xl border bg-background/50 p-3"><div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{item.label}</div><div className="mt-2 flex items-end justify-between gap-3"><div><div className="text-[9px] text-muted-foreground">Before</div><div className="text-xl font-black">{item.before}{item.suffix ?? ''}</div></div><div className="text-muted-foreground">→</div><div className="text-right"><div className="text-[9px] text-muted-foreground">Current</div><div className="text-xl font-black">{item.current}{item.suffix ?? ''}</div></div></div><div className={`mt-2 inline-flex items-center gap-1 text-[10px] font-bold ${delta === 0 ? 'text-muted-foreground' : improved ? 'text-emerald-600' : 'text-amber-600'}`}>{delta === 0 ? null : improved ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}{delta > 0 ? '+' : ''}{Number(delta.toFixed(1))}{item.suffix ?? ''}</div></div>
}
...
export function CisoExecutiveTimeline(props: CisoExecutiveTimelineProps) {
  ...
  const metrics: CisoTimelineMetric[] = [
    { label: 'Security score', before: Math.max(0, props.securityScore - props.scoreDelta), current: props.securityScore, inverse: true },
    { label: 'Risk exposure', before: Math.min(100, props.riskExposure + Math.max(0, props.scoreDelta * 2)), current: props.riskExposure },
    { label: 'Control coverage', before: Math.max(0, props.controlCoverage - props.scoreDelta), current: props.controlCoverage, suffix: '%', inverse: true },
    { label: 'Remediation rate', before: Math.max(0, props.remediationRate - props.scoreDelta), current: props.remediationRate, suffix: '%', inverse: true },
  ]

  return <section className="overflow-hidden rounded-2xl border bg-card shadow-sm" aria-label="CISO executive timeline">
    <header className="flex flex-wrap items-start justify-between gap-4 border-b px-5 py-4"><div><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground"><Activity className="h-3.5 w-3.5" /> CISO Executive Timeline</div><h2 className="mt-1 text-xl font-black tracking-tight">Outcome evolution at executive altitude</h2><p className="mt-1 max-w-3xl text-xs text-muted-foreground">A compact view of how measured risk, posture, assurance and governance signals are evolving. Unsupported historical events are not fabricated.</p></div><div className="rounded-full border px-3 py-1.5 text-[10px] font-bold">Live signal view</div></header>
