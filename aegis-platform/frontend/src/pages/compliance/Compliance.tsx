...
import { apiContractPaths, ComplianceValidationListSchema } from '@/contracts/api'
...
export const Compliance = () => {
  const query = useQuery<ComplianceItem[]>({ queryKey: ['compliance-overview'], queryFn: async () => {
    const vals = await apiHelpers.get<unknown>('/validations')
    const validations = Array.isArray(vals) ? vals : (vals && typeof vals === 'object' && ('items' in vals || 'results' in vals) ? ((vals as { items?: unknown[]; results?: unknown[] }).items ?? (vals as { results?: unknown[] }).results ?? []) : [])
    const id = (validations[0] as { id?: string } | undefined)?.id
    if (!id) return []
    const response = await apiHelpers.get<unknown>(apiContractPaths.validationCompliance(id))
    const raw = Array.isArray(response) ? response : (response && typeof response === 'object' && ('items' in response || 'results' in response) ? ((response as { items?: unknown[]; results?: unknown[] }).items ?? (response as { results?: unknown[] }).results ?? []) : [])
    return ComplianceValidationListSchema.parse(raw)
  } })
