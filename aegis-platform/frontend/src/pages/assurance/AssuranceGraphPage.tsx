...
export const AssuranceGraphPage = () => {
  const query = useQuery({ queryKey: ['assurance-graph'], queryFn: () => apiHelpers.get<any>('/assurance/correlations/conflicts') })
  ...
  if (query.error) return <div className="mx-auto grid min-h-[70vh] max-w-xl place-items-center p-6 text-center"><div><h1 className="text-lg font-bold">Assurance graph unavailable</h1><p className="mt-2 text-sm text-muted-foreground">Live correlation data could not be retrieved.</p><button type="button" onClick={() => query.refetch()} className="mt-4 inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold hover:bg-muted"><RefreshCw className="h-3.5 w-3.5" /> Retry</button></div></div>
  const graph = graphFromCorrelationPayload(query.data)
  const metrics = graphMetrics(graph)
  return <div className="space-y-5"><header><div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Assurance intelligence</div><h1 className="mt-1 text-2xl font-black tracking-tight">Unified Assurance Graph</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">A graph-native investigation surface connecting findings, evidence, confidence, and validation relationships.</p></header><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">{[['Nodes', metrics.nodes], ['Relations', metrics.edges], ['Conflicts', metrics.conflicts], ['Evidence-backed', metrics.evidenceBacked], ['Avg confidence', `${metrics.averageConfidence}%`], ['Critical', metrics.criticalNodes]].map(([label, value]) => <div key={String(label)} className="rounded-xl border bg-card p-4"><div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div><div className="mt-1 text-xl font-black">{value}</div></div>)}</div><AssuranceGraphView graph={graph} onInvestigate={(node) => { if (node.metadata?.validationId) window.location.assign(`/validations/${node.metadata.validationId}/results`) }} /></div>
}
