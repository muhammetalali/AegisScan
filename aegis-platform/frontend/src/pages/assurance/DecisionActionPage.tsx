...
export const DecisionActionPage = () => {
  ...
  return <div className="space-y-5"><header><div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Execution assurance</div><h1 className="mt-1 text-2xl font-black tracking-tight">Decision-to-Action Orchestration</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Own, execute, re-validate, and verify security decisions with an auditable lifecycle.</p></header>{actions.length ? <div className="space-y-5">{actions.map((action) => <DecisionActionOrchestration key={action.actionId} action={action} onTransition={(state) => void transition(action.actionId, state)} />)}</div> : <div className="rounded-2xl border bg-card p-10 text-center text-sm text-muted-foreground">No decision actions have been created yet. Create an action from a Security Decision.</div>}</div>
}
