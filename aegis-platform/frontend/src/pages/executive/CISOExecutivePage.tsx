...
export const CISOExecutivePage = () => {
  const query = useQuery({ queryKey: ['ciso-executive'], queryFn: async () => {
    const [summary, risk] = await Promise.all([apiHelpers.get<any>('/dashboard/summary'), apiHelpers.get<any>('/dashboard/risk-distribution')])
    return normalizeAssuranceModel(summary, risk)
  } })
  ...
  const model: ExecutiveSecurityModel = query.data
  return <CISOExecutiveView model={model} onOpenReports={() => { window.location.assign('/reports') }} />
}
