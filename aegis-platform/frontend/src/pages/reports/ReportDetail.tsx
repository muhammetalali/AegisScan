...
export const ReportDetail = () => {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-center justify-between gap-3"><Link to="/reports" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" /> Reports</Link><button type="button" onClick={()=>id&&window.open(`/api/v1/reports/${id}/download`,'_blank','noopener,noreferrer')} disabled={!id} className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold hover:bg-muted disabled:opacity-40"><Download className="h-3.5 w-3.5" /> Export</button></div><section className="rounded-2xl border bg-card p-6"><div className="flex items-start gap-3"><div className="grid h-11 w-11 place-items-center rounded-xl border bg-primary/5"><FileText className="h-5 w-5 text-primary" /></div><div><div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Assurance report</div><h1 className="mt-1 text-2xl font-bold">{title}</h1><p className="mt-1 text-sm text-muted-foreground">A report projection of the same security assurance model used by posture and executive views.</p></div></div><div className="mt-6 grid gap-3 md:grid-cols-4"><Metric label="Security score" value={show(model.securityScore) + '/100'} /><Metric label="Critical" value={show(model.critical)} /><Metric label="High" value={show(model.high)} /><Metric label="Control coverage" value={show(model.controlCoverage) + '%'} /></div></section><section className="grid gap-5 lg:grid-cols-2"><Panel title="Risk & assurance"><Row label="Risk exposure" value={show(model.riskExposure)} /><Row label="Validation coverage" value={show(model.validationCoverage) + '%'} /><Row label="Remediation rate" value={show(model.remediationRate) + '%'} /><Row label="Open exceptions" value={show(model.openExceptions)} /></Panel><Panel title="Report provenance"><div className="flex items-center gap-2 text-sm font-semibold"><ShieldCheck className="h-4 w-4 text-primary" /> Same source-of-truth model</div><p className="mt-2 text-xs leading-5 text-muted-foreground">Risk, findings, controls, evidence, validation, and executive impact converge on the normalized assurance context returned by the platform.</p></Panel></section></div>
}
