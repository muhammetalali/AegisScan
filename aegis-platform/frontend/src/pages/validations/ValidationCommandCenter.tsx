  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] space-y-5 pb-10">
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-2 flex flex-wrap gap-x-5 gap-y-2 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1.5"><Target className="h-3.5 w-3.5" /><span dir="ltr" className="font-mono text-foreground">{data.target_value ?? data.assets?.[0]?.name ?? '—'}</span></span>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard label="Evidence" value={safeNumber(overview?.evidence_count, data.evidences?.length)} icon={FileCheck2} tone="neutral" />
      </div>
...
const FindingRow = ({ finding }: { finding: any }) => { const key = (String(finding.severity ?? 'informational').toLowerCase() in severityMeta ? String(finding.severity).toLowerCase() : 'informational') as Severity; return <tr className="border-b last:border-0 hover:bg-muted/20"><td className="px-5 py-3"><span className={cn('inline-flex items-center gap-1.5 rounded-full border px-2 py-1 text-[10px] font-semibold', severityMeta[key].badge)}><span className={cn('h-1.5 w-1.5 rounded-full', severityMeta[key].dot)} />{severityMeta[key].label}</span></td><td className="px-5 py-3"><div className="max-w-[360px] truncate font-medium">{finding.title ?? finding.name ?? 'Untitled finding'}</div><div className="mt-1 text-[10px] text-muted-foreground">{finding.category ?? 'Security finding'}{finding.cwe ? ` • ${finding.cwe}` : ''}{finding.cvss ? ` • CVSS ${finding.cvss}` : ''}</div></td><td className="px-5 py-3 font-mono text-[11px]">{finding.asset ?? finding.asset_name ?? '—'}</td><td className="px-5 py-3">{finding.confidence != null ? `${finding.confidence}%` : '—'}</td><td className="px-5 py-3"><span className="rounded-lg border bg-muted/30 px-2 py-1 text-[10px] capitalize">{finding.status ?? 'open'}</span></td><td className="px-5 py-3 text-right"><Link to={`/vulnerabilities/${finding.id}`} className="inline-flex items-center gap-1 font-medium text-primary hover:underline">Inspect <ArrowRight className="h-3 w-3" /></Link></td></tr> }
