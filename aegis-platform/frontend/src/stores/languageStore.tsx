...
const CATALOGS: Record<Language, Catalog> = {
  ar: {
    AegisScan: 'AegisScan', 'Security Validation': 'التحقق الأمني', 'Security Validation Platform': 'منصة التحقق الأمني', Overview: 'نظرة عامة', Dashboard: 'لوحة التحكم', Workspace: 'مساحة العمل', Projects: 'المشاريع', Assets: 'الأصول', Validate: 'التحقق', 'New Validation': 'تحقق جديد', Validations: 'عمليات التحقق', Findings: 'الثغرات', Analyze: 'التحليل', Reports: 'التقارير', Compliance: 'الامتثال', 'Security Assurance': 'ضمان الأمن', 'Security Posture': 'الوضع الأمني', 'Digital Twin': 'التوأم الرقمي', Knowledge: 'المعرفة', Manage: 'الإدارة', 'Users & RBAC': 'المستخدمون والصلاحيات', 'Audit Trail': 'سجل التدقيق', Notifications: 'الإشعارات', System: 'النظام', Settings: 'الإعدادات', 'System Monitor': 'مراقبة النظام',
    Search: 'بحث', 'Search validations, findings, assets...': 'البحث في عمليات التحقق والثغرات والأصول...', 'View all': 'عرض الكل', Logout: 'تسجيل الخروج', 'Change language': 'تغيير اللغة',
...
const translateRenderedText = (language: Language) => {
  ...
  try {
    ...
    for (const textNode of nodes) {
      ...
      if (!raw || !parent || raw.trim().length === 0 || raw.trim().length > 400) continue
