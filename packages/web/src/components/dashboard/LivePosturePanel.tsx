...
import { Activity, ArrowUpRight, CheckCircle2, CircleAlert, ShieldCheck } from 'lucide-react'
import { apiHelpers } from '@/services/api'
import { cn } from '@/utils/cn'

interface Metric { name: string; value: number | null; percentage?: number | null; trend?: string; measured?: boolean }
interface LiveDashboard { posture: { score: number; rating: string; metrics: Metric[]; recommendations: string[]; trend: { direction: string; change_rate: number } }; recent_validations: Array<{ id?: string; target_value?: string; status?: string }> }
...
export const LivePosturePanel = () => {
  ...
  return <section className="aegis-surface p-5">
    <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
      <div><div className="aegis-kicker flex items-center gap-2 text-primary"><Activity className="h-3.5 w-3.5" />Live assurance state</div><div className="mt-1 flex items-end gap-3"><span className="text-3xl font-bold tracking-tight">{Math.round(posture.score)}</span><span className="pb-1 text-xs text-muted-foreground">/ 100 · {posture.rating}</span><span className="pb-1 text-xs font-medium text-muted-foreground">{posture.trend.direction} ({posture.trend.change_rate >= 0 ? '+' : ''}{posture.trend.change_rate})</span></div></div>
      <div className="flex items-center gap-2 rounded-xl border px-3 py-2 text-xs"><ShieldCheck className="h-4 w-4 text-primary" />Measured from validation evidence</div>
    </div>
