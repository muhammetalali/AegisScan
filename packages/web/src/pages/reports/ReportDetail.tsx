...
export const ReportDetail = () => {
  ...
  const downloadReport = async () => {
    if (!id || report?.status !== 'completed') return
    setDownloading(true)
    try {
      ...
      anchor.href = url
      anchor.download = `${title.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'aegisscan-report'}.${report.format ?? 'pdf'}`
