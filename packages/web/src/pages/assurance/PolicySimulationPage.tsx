...
export function PolicySimulationPage() {
  ...
  const run = async () => {
    if (!selectedPolicy || !actionId) {
      setError('A persisted policy and live action are required for simulation.')
      return
    }
    setError('')
    try {
      const response = await apiHelpers.post<Result>('/assurance/policies/simulate', { action_id: actionId, policy_id: selectedPolicy })
      setResult(response)
    } catch (cause) {
      setResult(null)
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border bg-card p-5"><div className="grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end">
      <label className="text-xs font-semibold">Action<select value={actionId} onChange={e => setActionId(e.target.value)} className="mt-2 w-full rounded-lg border bg-background px-3 py-2 text-sm font-normal">{actions.map(a => <option key={a.actionId} value={a.actionId}>{a.title}</option>)}</select></label>
