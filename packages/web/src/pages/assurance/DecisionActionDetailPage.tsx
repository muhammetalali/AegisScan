...
export const DecisionActionDetailPage = () => {
  ...
  const load = useCallback(async () => { try { const response = await fetch(`/api/v1/assurance/actions/${actionId}`, { headers: { Authorization: `Bearer ${token}` } }); if (!response.ok) throw new Error('Action unavailable'); setAction(await response.json()) } catch (err: any) { setError(err.message ?? 'Unable to load action') } }, [actionId, token])
