...
import { InvestigationGraphCommandLayer } from '@/components/security/InvestigationGraphCommandLayer'
...
export const AssuranceGraphPage = () => {
  ...
  const graph = query.data?.nodes && query.data?.edges ? query.data : graphFromCorrelationPayload(query.data)
  const metrics = graphMetrics(graph)
  const intelligence = query.data?.intelligence as GraphIntelligence | undefined
  const investigate = (node: any) => { if (node?.metadata?.validationId) window.location.assign(`/validations/${node.metadata.validationId}/results`) }
  return <div className="space-y-5"><header><div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Assurance intelligence</div><h1 className="mt-1 text-2xl font-black tracking-tight">Unified Assurance Graph</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Graph-native decision surface connecting findings, evidence, validations, conflicts, risk propagation, and executive priority.</p></header>{intelligence && <AssuranceGraphIntelligencePanel intelligence={intelligence} onInvestigate={(id) => investigate(graph.nodes?.find((item: any) => item.id === id))} />}<div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">{[['Nodes', metrics.nodes], ['Relations', metrics.edges], ['Conflicts', metrics.conflicts], ['Evidence-backed', metrics.evidenceBacked], ['Avg confidence', `${metrics.averageConfidence}%`], ['Critical', metrics.criticalNodes]].map(([label, value]) => <div key={String(label)} className="rounded-xl border bg-card p-4"><div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div><div className="mt-1 text-xl font-black">{value}</div></div>)}</div><InvestigationGraphCommandLayer graph={graph} onInvestigate={investigate} /></div>
}
