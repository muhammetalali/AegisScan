...
const emptyPolicy: Policy = {
  id: 'custom-policy',
  version: 1,
  name: 'New assurance policy',
  enabled: true,
  priority: 50,
  when: {},
  actions: {
    approval_role: 'security_manager',
    approval_count: 1,
    sla_hours: 24,
    escalate_after_minutes: 360,
    escalation_targets: ['security_manager'],
  },
}
...
export function PolicyStudioPage() {
  ...
  const load = useCallback(async () => {
    setError('')
    try {
      const response = await fetch('/api/v1/assurance/policies', { headers: { Authorization: `Bearer ${token}` } })
  ...
  const save = async () => {
    setError('')
    try {
      const response = await fetch(selected ? `/api/v1/assurance/policies/${selected}` : '/api/v1/assurance/policies', {
  ...
  const simulate = async () => {
    if (!selected) {
      setSimulation({ error: 'Select an existing policy and action to simulate.' })
      return
    }
