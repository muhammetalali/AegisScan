...
export function WorkflowControlTowerPage() {
 ...
 useEffect(()=>{void load(); const token=localStorage.getItem('access_token')||''; const protocol=location.protocol==='https:'?'wss':'ws'; const socket=new WebSocket(`${protocol}://${location.host}/ws/workflow?token=${encodeURIComponent(token)}`); socket.onopen=()=>setLive(true); socket.onclose=()=>setLive(false); socket.onmessage=()=>{void load()}; return()=>socket.close()},[])
