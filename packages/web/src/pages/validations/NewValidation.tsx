...
const ALL_ENGINES = ['recon','evidence_collection','vuln_intelligence','validation','control_validation','coverage_gap','attack_path','evidence_graph','knowledge','posture','policy_compliance','twin_engine','scenarios','dashboard','reporting']
...
export const NewValidation = () => {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<FormValues>({
    ...
    defaultValues: { target_type: 'url', target_value: '', profile: 'full', engines: ALL_ENGINES, scope_text: '', authorized: false, include_subdomains: false, duration_minutes: 60, rate_limit: 5, api_method: 'GET' },
  })
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const response = await apiHelpers.post<{ id?: string; validation_id?: string }>('/validations', payload)
      const id = response.id || response.validation_id
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
        ...
        <div className="text-sm"><p className="font-semibold">Authorized Security Testing Only</p><p className="mt-1 leading-relaxed text-muted-foreground">لا يبدأ أي تنفيذ إلا بعد تأكيد التصريح والنطاق. كل ما يظهر بعد الإنشاء مصدره Validation Runtime الحقيقي.</p></div>
      </motion.div>

      <header><h1 className="flex items-center gap-2 text-2xl font-bold"><ShieldCheck className="h-6 w-6 text-primary" /> New Security Validation</h1><p className="mt-1 text-sm text-muted-foreground">حدد الهدف والنطاق والمحركات ثم ابدأ التنفيذ الحقيقي.</p></header>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <section className="rounded-xl border bg-card p-5">
          <div className="mb-4 flex items-center gap-2 font-semibold"><span className="grid h-6 w-6 place-items-center rounded-full bg-primary text-xs text-primary-foreground">1</span> نوع الهدف</div>
        ...
        <section className="space-y-4 rounded-xl border bg-card p-5">
          <div className="flex items-center gap-2 font-semibold"><span className="grid h-6 w-6 place-items-center rounded-full bg-primary text-xs text-primary-foreground">2</span> الهدف والنطاق</div>
        ...
        <section className="rounded-xl border bg-card p-5">
          <div className="mb-4 flex items-center gap-2 font-semibold"><span className="grid h-6 w-6 place-items-center rounded-full bg-primary text-xs text-primary-foreground">3</span> ملف التحقق</div>
          <div className="grid gap-3 md:grid-cols-3">
            {(['quick','full','custom'] as const).map((value) => <button key={value} type="button" onClick={() => selectProfile(value)} className={cn('rounded-xl border p-4 text-start', profile === value ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'hover:bg-muted/50')}><div className="flex items-center justify-between"><span className="font-medium">{value === 'quick' ? 'Quick Assessment' : value === 'full' ? 'Full Validation' : 'Custom'}</span>{profile === value && <Check className="h-4 w-4 text-primary" />}</div><p className="mt-1 text-xs text-muted-foreground">{value === 'quick' ? 'مجموعة محركات أساسية' : value === 'full' ? 'جميع محركات المنصة المتاحة' : 'اختيار يدوي للمحركات'}</p></button>)}
          </div>
          <div className="mt-4 flex items-center justify-between"><span className="flex items-center gap-2 text-sm font-medium"><Layers className="h-4 w-4 text-primary" /> Engines ({engines.length}/15)</span><div className="flex gap-2"><button type="button" onClick={() => setValue('engines', ALL_ENGINES, { shouldValidate: true })} className="rounded border px-2 py-1 text-xs hover:bg-muted">الكل</button><button type="button" onClick={() => setValue('engines', [], { shouldValidate: true })} className="rounded border px-2 py-1 text-xs hover:bg-muted">مسح</button></div></div>
        ...
        <section className="rounded-xl border bg-card p-5"><div className="mb-4 flex items-center gap-2 font-semibold"><span className="grid h-6 w-6 place-items-center rounded-full bg-primary text-xs text-primary-foreground">4</span> حدود التنفيذ</div><div className="grid gap-3 sm:grid-cols-2"><div><label className="flex items-center gap-1 text-sm font-medium"><Clock className="h-3.5 w-3.5" /> المدة القصوى بالدقائق</label><input type="number" {...register('duration_minutes')} className="mt-1 w-full rounded-lg border bg-background px-3 py-2 text-sm" /></div><div><label className="flex items-center gap-1 text-sm font-medium"><Gauge className="h-3.5 w-3.5" /> Rate limit</label><input type="number" {...register('rate_limit')} className="mt-1 w-full rounded-lg border bg-background px-3 py-2 text-sm" /></div></div></section>

        <div className="flex items-center justify-between gap-3 pt-2"><p className="text-xs text-muted-foreground">Validate → Create Runtime Job → Live Progress → Results</p><button type="submit" disabled={submitting || !authorized} className="inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-2.5 font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50">{submitting ? <><Loader2 className="h-4 w-4 animate-spin" /> جاري الإنشاء…</> : <>Start Validation <ArrowRight className="h-4 w-4" /></>}</button></div>
      </form>
    </div>
  )
}
