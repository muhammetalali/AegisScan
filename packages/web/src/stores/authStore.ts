...
const browserStorage = {
  ...
  setPreference(mode: 'local' | 'session') {
    window.localStorage.setItem(AUTH_STORAGE_PREFERENCE, mode)
  },
  clearAll() {
...
const applyAuthorization = (token: string) => {
...
const extractApiMessage = (error: unknown, fallback: string): string => {
  ...
  if (response && typeof response === 'object') {
    ...
    for (const key of ['detail', 'message', 'error']) {
      if (typeof data[key] === 'string' && String(data[key]).trim()) return String(data[key])
    }
...
export const useAuthStore = create<AuthState>()(persist((set, get) => ({
  ...
  login: async (email, password, rememberMe = true, otp) => {
    ...
    try {
      ...
      if (data?.two_factor_required && !data?.access) {
        set({ loading: false })
        return
      }
  ...
  register: async (data) => {
    ...
    try {
      ...
      browserStorage.setPreference('local')
      window.sessionStorage.removeItem(AUTH_STORAGE_KEY)
      ...
      clearAuthorization()
      set({ loading: false, isAuthenticated: false, error: extractApiMessage(error, 'Registration failed') })
      throw error
    }
  },
  ...
  logout: async () => {
    ...
    try {
      if (refreshToken) await api.post('/users/logout/', { refresh_token: refreshToken })
    } catch {
      // Local session teardown remains authoritative when the server is unavailable.
    } finally {
      clearAuthorization()
  ...
  forgotPassword: async (email) => {
    set({ loading: true, error: null })
    try { await api.post('/auth/password/reset/', { email: email.trim().toLowerCase() }); set({ loading: false }) }
    catch (error) { set({ loading: false, error: extractApiMessage(error, 'Failed to send reset email') }); throw error }
  },
  resetPassword: async (token, password) => {
    set({ loading: true, error: null })
    try { await api.post('/auth/password/reset/confirm/', { token, password }); set({ loading: false }) }
    catch (error) { set({ loading: false, error: extractApiMessage(error, 'Failed to reset password') }); throw error }
  },
  verifyEmail: async (token) => {
    set({ loading: true, error: null })
    try { await api.post('/auth/verify-email/', { token }); set({ loading: false }) }
    catch (error) { set({ loading: false, error: extractApiMessage(error, 'Failed to verify email') }); throw error }
  },
  resendVerification: async () => {
    set({ loading: true, error: null })
    try { await api.post('/auth/resend-verification/'); set({ loading: false }) }
    catch (error) { set({ loading: false, error: extractApiMessage(error, 'Failed to resend verification') }); throw error }
  },
  updateProfile: async (data) => {
    set({ loading: true, error: null })
    try { const response = await api.patch('/users/me/', data); set({ user: response.data, loading: false }) }
    catch (error) { set({ loading: false, error: extractApiMessage(error, 'Failed to update profile') }); throw error }
  },
  changePassword: async (oldPassword, newPassword) => {
    set({ loading: true, error: null })
    try { await api.post('/users/change_password/', { old_password: oldPassword, new_password: newPassword, new_password_confirm: newPassword }); set({ loading: false }) }
    catch (error) { set({ loading: false, error: extractApiMessage(error, 'Failed to change password') }); throw error }
  },
  ...
  fetchUser: async () => {
    ...
    if (!response.data) throw new Error('Invalid user response')
...
export const initAuth = async () => {
  ...
  try { await fetchUser() }
    ...
    if (refreshToken) { try { await refreshAccessToken(); await fetchUser() } catch { await logout() } }
    else await logout()
  }
}
