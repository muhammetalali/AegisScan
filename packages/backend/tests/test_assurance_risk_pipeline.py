            ...
            "items": [
                {
                    "source": "greynoise",
                    "confidence": 0.9,
                    "provenance": {"classification": "malicious", "noise": True, "riot": False},
                },
                {
                    "source": "shodan",
                    "confidence": 0.9,
                    "provenance": {"ports": [22, 443, 8080]},
                },
            ],
        }
...
class FakeRemediation:
    async def validate_workspace(self, candidate, *, tools, timeout):
        return {
            "passed": True,
            "blocked": False,
            "summary": {"requested": len(tools or ["semgrep"]), "available": len(tools or ["semgrep"]), "failed": 0, "missing": 0},
            "tools": [],
        }
...
async def test_pipeline_correlates_cti_and_remediation():
    pipeline = AssuranceRiskPipeline(
...
async def test_pipeline_propagates_validation_regression():
    ...
    pipeline = AssuranceRiskPipeline(
        intelligence=FakeIntelligence(),
        external=FakeExternal(),
        remediation=RegressedRemediation(),
    )
