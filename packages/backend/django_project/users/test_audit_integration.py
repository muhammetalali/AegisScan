...
class UserAuditIntegrationTests(APITestCase):
    ...
    def test_successful_login_creates_audit_event(self):
        ...
        event = AuditLog.objects.get(action=AuditLog.Action.LOGIN, result=AuditLog.Result.SUCCESS)
    ...
    def test_failed_login_creates_audit_event_without_credentials(self):
        ...
        event = AuditLog.objects.filter(action=AuditLog.Action.LOGIN, result=AuditLog.Result.FAILURE).latest('created_at')
        self.assertEqual(event.user_id, self.user.id)
        self.assertEqual(event.metadata.get('reason'), 'invalid_credentials')
        self.assertNotIn('wrong-password', str(event.metadata))

    def test_failed_unknown_user_login_creates_anonymous_audit_event(self):
        response = self.client.post(
            self.login_url,
        ...
        self.assertEqual(event.metadata.get('reason'), 'authentication_failed')
    ...
    def test_password_change_creates_audit_event(self):
        ...
        self.assertEqual(response.status_code, 200)
        event = AuditLog.objects.get(action=AuditLog.Action.PASSWORD_CHANGE, result=AuditLog.Result.SUCCESS)
        self.assertEqual(event.user_id, self.user.id)
    ...
    def test_logout_creates_audit_event(self):
        ...
        self.assertEqual(response.status_code, 200)
        event = AuditLog.objects.get(action=AuditLog.Action.LOGOUT, result=AuditLog.Result.SUCCESS)
        self.assertEqual(event.user_id, self.user.id)
    ...
    def test_register_creates_audit_event(self):
        ...
        event = AuditLog.objects.get(action=AuditLog.Action.USER_CREATE, result=AuditLog.Result.SUCCESS, user=user)
        self.assertEqual(event.resource_id, str(user.pk))

    def test_audit_hash_chain_is_populated_after_user_events(self):
        self.client.post(
            self.login_url,
            {'email': self.user.email, 'password': 'wrong-password'},
            format='json',
        ...
        events = list(AuditLog.objects.filter(user=self.user).order_by('sequence'))
        ...
        for previous, current in zip(events, events[1:]):
            ...
            self.assertEqual(len(current.entry_hash), 64)
