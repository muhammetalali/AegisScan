...
class ProjectAuthorization:
    ...
    def can_delete(self) -> bool:
        return self.role in OWNER_ONLY_ROLES
    ...
    def can_manage_membership(self, target: ProjectMembership, *, new_role: str | None = None) -> bool:
        ...
        Admins cannot modify/remove the project owner or grant owner privileges.
        Owners may manage every membership, including changing roles.
        """
        if not self.can_manage_members:
            return False

        if self.role == ProjectMembership.Role.OWNER:
            return True
