...
from .authorization import ProjectAuthorization, get_project_authorization
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def _authorization(self, project: Project) -> ProjectAuthorization:
        authz = get_project_authorization(project.pk, self.request.user)
        if project.owner_id == self.request.user.id and not authz.membership:
            owner_membership = ProjectMembership(
                project=project,
                user=self.request.user,
                role=ProjectMembership.Role.OWNER,
            )
            return ProjectAuthorization(owner_membership)
        return authz
    ...
    def perform_create(self, serializer):
        with transaction.atomic():
            ...
            _audit(
                self.request,
                action=AuditLog.Action.PROJECT_CREATE,
                project=project,
    ...
    def perform_update(self, serializer):
        ...
        tracked_fields = (
            "name", "slug", "description", "status", "environment", "tags", "settings", "default_scan_config"
        )
        before = {field: getattr(project, field) for field in tracked_fields}
    ...
    def archive(self, request, pk=None):
        ...
        if project.status != Project.Status.ARCHIVED:
            ...
            _audit(
                request,
                action=AuditLog.Action.PROJECT_ARCHIVE,
                project=project,
                changes={"status": Project.Status.ARCHIVED},
            )
    ...
    def members(self, request, pk=None):
        ...
        _audit(
            request,
            action=AuditLog.Action.PROJECT_MEMBER_ADD,
            project=project,
            changes={"user_id": str(target_user.pk), "role": role},
        )
        return Response(
            {"id": str(membership.pk), "user_id": str(target_user.pk), "role": membership.role},
    ...
    def member_detail(self, request, pk=None, user_id=None):
        ...
        if request.method == "DELETE":
            ...
            _audit(
                request,
                action=AuditLog.Action.PROJECT_MEMBER_REMOVE,
                project=project,
                changes={"user_id": str(membership.user_id), "role": membership.role},
            )
            membership.delete()
        ...
        _audit(
            request,
            action=AuditLog.Action.PROJECT_MEMBER_ROLE_CHANGE,
            project=project,
            changes={"user_id": str(membership.user_id), "from": old_role, "to": new_role},
        )
