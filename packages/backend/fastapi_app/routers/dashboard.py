...
def _dashboard_snapshot(user_id: str, days: int = 30, limit: int = 5) -> dict:
    ...
    scores = [v for v in completed.order_by("-created_at").values_list("security_score", flat=True)[:10] if v is not None]
    ...
    assessed_controls = sum(int(compliance.get(k) or 0) for k in ("compliant", "non_compliant", "partial"))
    compliance_score = round((int(compliance.get("compliant") or 0) / assessed_controls) * 100) if assessed_controls else None
    ...
    return {
        "summary": {
            "security_score": round(sum(float(v) for v in scores) / len(scores)) if scores else None,
...
async def dashboard_summary(user_id: str = Depends(current_user_id)):
    return DashboardSummary(**(await _dashboard_snapshot(user_id))["summary"])
