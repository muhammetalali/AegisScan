...
def _access_limited(status_code: int, headers: dict[str, str]) -> tuple[bool, str | None]:
    if status_code not in {401, 403, 429}:
        return False, None
    server = headers.get("server", "").lower()
    blocked_by = None
    if "cloudflare" in server or headers.get("cf-mitigated") or "cf-ray" in headers:
        blocked_by = "cloudflare"
    elif status_code == 429:
        blocked_by = "rate_limit"
    elif status_code in {401, 403}:
        blocked_by = "access_control"
...
async def execute_http_probe(target_type: str, target_value: str, timeout: float = 15.0) -> ExecutionResult:
    ...
    # A 401/403/429 generated by an access-control layer does not prove
    # anything about the protected origin's security headers. Preserve the
    # real HTTP evidence and report the limitation, but do not manufacture
    # origin findings from the block/challenge response.
    ...
    return ExecutionResult(
        ...
        metrics={
            ...
            "access_limited": access_limited,
            "access_limited_by": access_limited_by,
            "assessment_scope": "edge_response_only" if access_limited else "direct_http_response",
        },
    )