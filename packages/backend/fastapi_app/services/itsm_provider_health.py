...
async def _check_jira(timeout: float) -> dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, auth=(email, token), headers={"Accept": "application/json"}) as client:
        ...
        return {"status": "healthy", "http_status": response.status_code, "latency_ms": latency_ms, "account_id_present": bool(data.get("accountId"))}
    except httpx.TimeoutException:
        return {"status": "timeout", "latency_ms": round((time.perf_counter() - started) * 1000, 2)}
    except httpx.HTTPError as exc:
        return {"status": "transport_error", "latency_ms": round((time.perf_counter() - started) * 1000, 2), "error_type": type(exc).__name__}
...
async def _check_servicenow(timeout: float) -> dict[str, Any]:
    ...
    started = time.perf_counter()
    try:
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, auth=auth, headers=headers) as client:
            response = await client.get(f"{base}/api/now/table/{table}", params={"sysparm_limit": "1", "sysparm_fields": "sys_id"})
        latency_ms = round((time.perf_counter() - started) * 1000, 2)
        if response.status_code in {401, 403}:
            return {"status": "auth_failed", "http_status": response.status_code, "latency_ms": latency_ms}
        if response.status_code == 404:
...
async def check_provider(provider: str, timeout: float = 8.0) -> dict[str, Any]:
    ...
    if not state.valid:
        return {"provider": provider, "status": "invalid_configuration", "configured": True, "errors": list(state.errors)}
...
async def check_all_providers(timeout: float = 8.0) -> dict[str, Any]:
    results = {provider: await check_provider(provider, timeout) for provider in ("jira", "servicenow")}
    healthy = all(item["status"] in {"healthy", "not_configured"} for item in results.values())
    return {"status": "healthy" if healthy else "degraded", "providers": results}
