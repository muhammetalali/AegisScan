...
def _event_row(row: tuple[Any, ...]) -> dict[str, Any]:
    return {"type": row[0], "at": row[1].isoformat(), "actor": row[2], "note": row[3]}


def _hydrate(cur, row: tuple[Any, ...]) -> dict[str, Any]:
    (
        action_id, decision_id, node_id, title, owner, requested_by, sla_hours, state,
        risk_before, confidence_before, priority, recommended_action, remediation_plan,
        created_at, updated_at, version, sla_status, escalation_level, idempotency_key,
    ) = row
    ...
    return {
        "actionId": action_id, "decisionId": decision_id, "nodeId": node_id, "title": title,
        "owner": owner, "requestedBy": requested_by, "slaHours": sla_hours, "state": state,
        "riskBefore": risk_before, "confidenceBefore": confidence_before, "priority": priority,
        "recommendedAction": recommended_action,
        "remediationPlan": remediation_plan if isinstance(remediation_plan, list) else json.loads(remediation_plan or "[]"),
        "createdAt": created_at.isoformat(), "updatedAt": updated_at.isoformat(), "dueAt": due_at.isoformat(),
        "version": version, "slaStatus": sla_status, "escalationLevel": escalation_level,
        "events": events, "idempotencyKey": idempotency_key,
    }


def create_action(
    decision: dict[str, Any], owner: str, sla_hours: int, requested_by: str, idempotency_key: str | None = None,
) -> dict[str, Any]:
    _ensure_schema()
    ...
    try:
        with conn.cursor() as cur:
            ...
            values = {
                ...
                "remediation_plan": json.dumps(decision.get("revalidationPlan", [])),
                "created_at": now,
                "updated_at": now,
                "idempotency_key": idempotency_key,
            }
            cur.execute(
                """INSERT INTO security_decision_actions (
                    action_id, decision_id, node_id, title, owner, requested_by,
                    sla_hours, state, risk_before, confidence_before, priority,
                    recommended_action, remediation_plan, created_at, updated_at, idempotency_key
                ) VALUES (
                    %(action_id)s, %(decision_id)s, %(node_id)s, %(title)s, %(owner)s, %(requested_by)s,
                    %(sla_hours)s, %(state)s, %(risk_before)s, %(confidence_before)s, %(priority)s,
                    %(recommended_action)s, %(remediation_plan)s, %(created_at)s, %(updated_at)s, %(idempotency_key)s
                )""",
                values,
            )
            cur.execute(
                "INSERT INTO security_decision_action_events (action_id,event_type,actor,created_at) VALUES (%s,%s,%s,%s)",
                (action_id, "action.created", requested_by, now),
            )
            cur.execute("SELECT * FROM security_decision_actions WHERE action_id=%s", (action_id,))
            row = cur.fetchone()
            item = _hydrate(cur, row)
            conn.commit()
            return item
    finally:
        pool.putconn(conn)
...
def transition(
    action_id: str, state: str, actor: str, note: str | None = None, *, verification_context: bool = False,
) -> dict[str, Any]:
    _ensure_schema()
    if state not in STATES:
        raise ValueError(f"Invalid state: {state}")
    if state == "verified" and not verification_context:
        raise ValueError("verified is only reachable through remediation verification")
    pool = _pool_instance()
    conn = pool.getconn()
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT state, version FROM security_decision_actions WHERE action_id=%s FOR UPDATE", (action_id,))
            row = cur.fetchone()
            if row is None:
            ...
            now = _now()
            cur.execute(
                "UPDATE security_decision_actions SET state=%s,updated_at=%s,version=version+1 WHERE action_id=%s AND version=%s",
                (state, now, action_id, version),
            )
            if cur.rowcount != 1:
                raise RuntimeError("Concurrent action update detected")
            cur.execute(
                "INSERT INTO security_decision_action_events(action_id,event_type,actor,note,created_at) VALUES(%s,%s,%s,%s,%s)",
                (action_id, f"action.{state}", actor, note, now),
            )
            cur.execute("SELECT * FROM security_decision_actions WHERE action_id=%s", (action_id,))
            item = _hydrate(cur, cur.fetchone())
            conn.commit()
            return item
    finally:
        pool.putconn(conn)
...
def list_actions() -> list[dict[str, Any]]:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM security_decision_actions ORDER BY updated_at DESC")
            return [_hydrate(cur, row) for row in cur.fetchall()]
    finally:
        pool.putconn(conn)
...
def get_action(action_id: str) -> dict[str, Any] | None:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM security_decision_actions WHERE action_id=%s", (action_id,))
            row = cur.fetchone()
            return _hydrate(cur, row) if row else None
    finally:
        pool.putconn(conn)
