...
def test_origin_requires_verified_https_and_rejects_loopback():
    ...
    assert acceptance._origin("https://[fd12:3456::10]:8443", False) == "https://[fd12:3456::10]:8443"
...
def test_internal_dns_must_resolve_exclusively_to_private_addresses(monkeypatch):
    ...
    assert acceptance._resolved_internal_addresses("https://security.internal") == [
        "10.20.30.40",
        "192.168.4.8",
    ]
...
def test_enterprise_ca_bundle_is_required_absolute_and_bounded(tmp_path: Path, monkeypatch):
    ...
    empty = tmp_path / "empty-ca.pem"
    empty.write_bytes(b"")
    monkeypatch.setenv("AEGIS_ENTERPRISE_CA_BUNDLE", str(empty))
    with pytest.raises(acceptance.AcceptanceError, match="invalid size"):
        acceptance._ca_bundle_path()
...
def _success_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        acceptance,
        "_resolved_internal_addresses",
        lambda *args, **kwargs: ["10.20.30.40"],
    )
    monkeypatch.setattr(
        acceptance,
        "_ca_bundle_evidence",
        lambda: (Path("/etc/aegisscan/enterprise-ca.pem"), "c" * 64),
    )
    monkeypatch.setattr(
        acceptance,
        "_request",
...
def test_validate_requires_health_ready_frontend_headers_private_dns_and_enterprise_ca(monkeypatch):
    _success_dependencies(monkeypatch)
    result = acceptance.validate("https://security.internal")
    assert result["schema"] == "aegisscan.internal-production-acceptance.v1"
